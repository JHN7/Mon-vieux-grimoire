...
exports.createBook = (req, res, next) => {
    try {
        // Vérifie que le fichier image est bien présent
        if (!req.file) {
            return res.status(400).json({ message: 'L’image du livre est obligatoire.' });
        }
        ...
        console.error('Erreur serveur dans createBook:', error);
    ...
    Book.findOne({ _id: req.params.id })
        .then((book) => {
            ...
            if (book.userId != req.auth.userId) {
                ...
                Book.updateOne({ _id: req.params.id }, { ...bookObject, _id: req.params.id }, { runValidators: true })
